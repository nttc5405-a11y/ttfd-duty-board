...
注意一：不要用正則 /\*.*?\*/ 移除區塊註解 —— 程式碼字串裡若出現 "*/"
（例如 Accept 標頭的 */*），會被誤判成註解結尾，把中間整段程式碼吃掉。
這裡改為逐行判斷：只有「行首」是 /* 才算註解開始。

注意二：壓縮後所有行會接成「一行」、沒有換行符。若原始碼裡有「行尾註解」
（程式碼後面接 // 說明，而不是整行都是註解），// 會把同一行之後、
已被接上的所有後續程式碼一併吃掉且不報錯。本工具會檢查並警告這種寫法，
但寫可讀版時仍應避免行尾註解，一律把註解獨立成一行。
...
def minify(src: str) -> str:
    ...
    for line in src.split("\n"):
        ...
        out.append(s)
        # 危險寫法偵測：程式碼後面接了行尾 // 註解。壓縮後沒有換行符，
        # 這種註解會把「同一行之後、已被接上的所有後續程式碼」一併吃掉。
        # 用 :// 排除網址（http://、https://）這種常見的無害情況。
        without_urls = s.replace("://", "")
        if "//" in without_urls:
            print("警告：偵測到疑似行尾註解，請改成獨立一行 -> %r" % s)
