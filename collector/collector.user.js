...
(function () {
  ...
  function ui() {
    ...
    head.appendChild(document.createTextNode("勤務看板採集器（自動模式 v14）"));
  ...
  //
  // 另外，表格本身是可以獨立捲動的區塊，一次只會渲染看得到的那幾
  // 列——單位數少（例如成功大隊只有 6 個）通常一次就顯示完，但單位
  // 數一多（見 collector-county.js，全縣 34 個）畫面只會先渲染前面
  // 十幾列，其餘要捲動才會出現，光是等待穩定不會讓其餘列自己長出
  // 來。正解：找到表格的捲動容器，主動一段一段往下捲，每捲一段就
  // 讀一次目前看得到的內容並累加進同一份對照表，直到捲到底或連續
  // 幾次捲不動為止，最後把捲動位置還原、不留痕跡；找不到可捲動
  // 容器（單位少、一次就顯示完）時，退回原本「輪詢等內容穩定」的
  // 做法。
